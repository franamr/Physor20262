...
def gen_power_it_dynamic_momentum(A, B, v0, tol=1e-8, max_iter=1000):
    ...
    Bx = B.createVecLeft()
    B.mult(x1, Bx)
    lam1 = float((x1.dot(Ax)/x1.dot(Bx)).real)
    ...
    Bx = B.createVecLeft()
    B.mult(x2, Bx)
    lam2 = float((x2.dot(Ax)/x2.dot(Bx)).real)
    ...
    while k < max_iter:
        ...
        Bx = B.createVecLeft()
        B.mult(x_kp1, Bx)
        lam_kp1 = float((x_kp1.dot(Ax) / x_kp1.dot(Bx)).real)
